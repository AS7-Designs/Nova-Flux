...
export default function NotFound() {
  ...
  return (
    <section className="section-padding relative container flex min-h-screen items-center justify-center overflow-hidden">
      <FluxDotGrid
        spacing={18}
        className="[mask-image:radial-gradient(ellipse_50%_50%_at_50%_50%,black,transparent)] opacity-[0.03] dark:opacity-[0.05]"
      />
      ...
      >
        <motion.h1 className="text-foreground mb-4 text-3xl font-bold md:text-4xl lg:text-5xl" variants={titleVariants}>
          Page Not Found
        </motion.h1>
        ...
        <motion.div variants={buttonVariants}>
          <Button asChild size="lg" variant="light" className="group !pl-5.5 font-medium">
            <Link href="/">
              Return Home
              <div className="bg-border border-input grid size-5.5 place-items-center rounded-full border">
                <AltArrowRightLinear className="size-4 transition-transform group-hover:translate-x-0.25" />
              </div>
            </Link>
          </Button>
        </motion.div>
